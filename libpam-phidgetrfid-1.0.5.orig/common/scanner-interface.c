 ...
 */
static int tag_handler(/*@unused@*/ CPhidgetRFIDHandle phidget_handle, /*@unused@*/ void *usrptr, unsigned char *TagVal)
{
	int i = 0;
...
int scan_tag(char *destination[], unsigned int timeout, unsigned int sleep_interval)
{
	...
	{
		//enable LED
		status = CPhidgetRFID_getLEDOn(_rfid_handle, &status);
		if(status == PTRUE)
		{
			(void)snprintf(_message, _max_message_length, "LED is already active");
			_log_function(LOG_DEBUG, _message);

		}
		else if(status == PFALSE )
		{
			(void)snprintf(_message, _max_message_length, "LED inactive: activating");
			_log_function(LOG_DEBUG, _message);
			status = CPhidgetRFID_setLEDOn(_rfid_handle, PTRUE);
			if(status != 0)
			{
		...
		{
			(void)snprintf(_message, _max_message_length, "error getting LED activation status: error code '%i'", status);
			_log_function(LOG_ERR, _message);
		}
		...
		*destination = (char *)malloc(RFID_TAG_LEN * (sizeof (char) + 1));
		...
		//disable LED
		status = CPhidgetRFID_setLEDOn(_rfid_handle, PFALSE);
		if(status != 0)
		{
			(void)snprintf(_message, _max_message_length, "error turning LED off: error code '%i'", status);
			_log_function(LOG_ERR, _message);
		}
